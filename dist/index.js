...
var turnMethods = {
    ...
    if (display) {
      ...
      if (display == "single") {
        if (!data.pageObjs[0]) {
          ...
          data.pageObjs[0] = $("<div />", { "class": "turn-page p-temporal" }).css({ width: this.width(), height: this.height() }).appendTo(this);
        }
      } else {
        if (data.pageObjs[0]) {
  ...
  _released: function(e, point) {
    ...
    e.stopPropagation();
    if ((/* @__PURE__ */ new Date()).getTime() - data.time < 200 || point.x < 0 || point.x > $(this).width()) {
      e.preventDefault();
...
var isTouchDevice = Boolean($.isTouch);

export { createTurn, isTouchDevice, useTurn };
